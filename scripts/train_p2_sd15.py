...
def main(args):
    ...
    text_encoder = text_encoder_cls_one.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision)
    ...
    # Adapter 
    config = UNITY_Config(ref_in_channel=3, source_in_channel=3, head_nums=3, downsample=8)
    Adapter = UNITY(config)
    if args.pretrained_adapter_model_name_or_path is not None:
        Adapter = Adapter.from_pretrained(args.pretrained_adapter_model_name_or_path)
        Adapter.set_input_channels(ref_in_channel=3, source_in_channel=3)
        Adapter.config.update(vars(config))
    else:
