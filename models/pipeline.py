...
class StableDiffusionAdapterPipeline(DiffusionPipeline, StableDiffusionMixin, FromSingleFileMixin):
    ...
    def __init__(
        ...
        adapter: Union[T2IAdapter, MultiAdapter, List[T2IAdapter], UNITY],
    ...
    def __call__(
        ...
        prompt2adapter: Union[str, List[str]] = None,
        image = None, # : Union[torch.Tensor, PIL.Image.Image, List[PIL.Image.Image]]
        ...
        if isinstance(self.adapter, MultiAdapter):
            ...
            for one_image in image:
                ...
                one_image = one_image.to(device=device, dtype=self.unet.dtype)
                adapter_input.append(one_image)
        if isinstance(self.adapter, UNITY):
            adapter_input = []
            for one_image in image:
                one_image = _preprocess_adapter_image(one_image, height, width)
                one_image = one_image.to(device=device, dtype=self.unet.dtype)
                adapter_input.append(one_image)
            adapter_input = torch.cat(adapter_input, dim=1)
        # print(adapter_input.shape)
            

        self.adapter = self.adapter.to(device=device, dtype=self.unet.dtype)
        ...
        p2a_prompt_embeds = prompt_embeds
        
        # p2a_prompt_embeds, p2a_negative_prompt_embeds = self.encode_prompt(
        #     prompt2adapter,
        #     device,
        #     num_images_per_prompt,
        #     self.do_classifier_free_guidance,
        ...
        if isinstance(self.adapter, MultiAdapter):
            ...
            # print("Single Adapter", prompt_embeds.shape)
            adapter_state = self.adapter(conditional_images=adapter_input, 
                                         prompt_embeds=p2a_prompt_embeds)
            for k, v in enumerate(adapter_state):
